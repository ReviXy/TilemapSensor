...
public class GridMapperLiteEditor : Editor
{
    ...
    SerializedProperty consider_agent_tile_an_observation;
    ...
    public override void OnInspectorGUI()
    {
        ...
        //Consider agent tile an observation
        EditorGUILayout.BeginHorizontal();
        EditorGUILayout.LabelField("Consider agent tile an observation:", GUILayout.MaxWidth(200));
        consider_agent_tile_an_observation.boolValue = EditorGUILayout.Toggle(consider_agent_tile_an_observation.boolValue);
        EditorGUILayout.EndHorizontal();
        ...
        for (int i = 0; i < row.intValue; i++)
        {
            ...
            for (int j = 0; j < column.intValue; j++)
            {
                ...
                switch (grid[i, j])
                {
                    ...
                    case 2: buttonColor = Color.red; buttonString = "⊠"; break;

                    case 3: grid[i, j] = 0; break;
                }
                ...
                if (GUILayout.Button(buttonString, gridButtonStyle, GUILayout.MinWidth(width / column.intValue)))
                {
                    ...
                    layoutHelpString += "(" + (j + 1) + "/" + (row.intValue - i) + ")";
                    gridArray.GetArrayElementAtIndex(j * row.intValue + i).intValue = grid[i, j];
